function WebsocketPlayer(canvas, canvasCtx) {
    this.videoPlayer;
    this.audioPlayer;
    this.videoCanvasCtx = canvasCtx;
    ...
    //incoming buffering and lip-sync flags
...
WebsocketPlayer.prototype.init = function(configuration) {
    //init audio player
    try {
    ...
    //init video
    try {
        this.videoPlayer = new jsmpeg(null, {canvas: this.videoCanvas, forceCanvas2D: true, ondecodeframe: this.videoDecodedCallback.bind(this)});
        this.videoPlayer.initSocketClient(null, configuration.videoWidth, configuration.videoHeight);
    } catch (e) {
...
WebsocketPlayer.prototype.requestVideoFrameCallback = function (timestamp) {
 ...
 * R R R R TYP S T
 * R - reserved
 * TYP - 2 bit data type, 01 - video, 10 - audio, 11 - rtcp
 * S - ssrc flag, 1 if ssrc present
 * T - timestamp flag, 1 if timestamp present
 */
WebsocketPlayer.prototype.onDataReceived = function(event) {
    var view = new DataView(event.data);
    var buffEntry = {};
    //de-multiplexing
    var stripBytes = 1;
    var header = view.getUint8(0);
    var packetType = header >> 2 & 0x03;
    var ssrc = header >> 1 & 0x01;
    var timestamp = header & 0x01;
    if (packetType == 1) {
        //video
        if (ssrc == 1) {
        ...
        if (!this.bufferFilled) {
            ...
            //just got sync and full buffer, flush it
            ...
            //we shouldn't hit this at any case
            buffEntry.timestamp = this.lastAudioTimestamp;
            buffEntry.data = this.stripBytes(new Uint8Array(event.data), stripBytes);
            this.incomingAudioBuffer.push(buffEntry);
        }
        ...
        //rtcp
        this.handleRtcp(this.stripBytes(new Uint8Array(event.data), stripBytes))
    } else {
        console.log("Unknown binary data received, type " + packetType);
    }

};

WebsocketPlayer.prototype.handleVideo = function(data) {
    this.videoPlayer.receiveSocketMessage(data);
};

WebsocketPlayer.prototype.handleAudio = function(data) {
    this.audioPlayer.ulaw8000(data);
};

WebsocketPlayer.prototype.handleRtcp = function(data) {
...
WebsocketPlayer.prototype.stop = function() {
    this.audioPlayer.stop();
    this.videoPlayer.stop();
    this.videoStarted = false;
};

WebsocketPlayer.prototype.stripBytes = function(data, length) {
    var ret = new Uint8Array(data.byteLength - length);
    for (var i = 0; i < ret.byteLength; i++, length++) {
        ret[i] = data[length];
    }
    return ret;
};

WebsocketPlayer.prototype.ntpFractionToMilliseconds = function(fraction) {
    return fraction * 1000 / 4294967296;
};

WebsocketPlayer.prototype.tsToMilliseconds = function(ts, rate) {
    return ts/rate * 1000;
};

WebsocketPlayer.prototype.toCommonTimeBase = function(isAudio, ts) {
...
WebsocketPlayer.prototype.getAudioPlayerTime = function() {
...
function AudioPlayer(audioContext, internalBufferSize, decodedBufferSize) {
    ...
    this.audioJSNode.onaudioprocess = function(event) {
        ...
        if (me.decodedBufferPos >= bufferSize) {
            decoded = me.shiftDecodedData(bufferSize);
            ...
            var missingSamplesLength = bufferSize - me.decodedBufferPos;
            console.log("Not enough data in audio buffer! Playing silence samples, size " + missingSamplesLength);
            decoded = me.shiftDecodedData(me.decodedBufferPos);
            output = event.outputBuffer.getChannelData(0);
...
AudioPlayer.prototype.pushDecodedData = function(data) {
    ...
    if (this.decodedBufferSize / 4 - this.decodedBufferPos > data.length) {
        ...
        console.log("ERROR Decoded audio buffer full, shift data and time. This can result in audio glitch");
        var avail = this.decodedBufferSize / 4 - this.decodedBufferPos;
        var shift = data.length - avail;
        this.shiftDecodedData(shift);
        for (i = 0; i < data.length; i++) {
            this.decodedBufferView[this.decodedBufferPos++] = data[i];
        }
        this.timePlaying += shift / 44100 * 1000;
    }
};
...
G711U.prototype.ulaw2linear = function(data) {
    var t;
    data = ~data;
    t = ((data & this.QUANT_MASK) << 3) + this.BIAS;
    t <<= (data & this.SEG_MASK) >> this.SEG_SHIFT;
    return((data & this.SIGN_BIT) != 0 ? (this.BIAS - t) : (t - this.BIAS));
